...
gen_dataset(DATA_PATH, 10)

images = []
for root, directories, files in os.walk(DATA_PATH):
    for file in tqdm(files, desc='Solving CAPTCHAS'):
        img = cv2.imread(os.path.join(root, file))
        
        # pil_img = Image.fromarray(img)
        # pil_img = pil_img.resize((135,50), Image.BILINEAR)
        ...
        print(f"actual content (filename): {file.split(sep='.')[0]}")
        print('-----------------------------------')
