...
font = ImageFont.truetype('Vera.ttf', size=52)
...
def gen_string(size=5, chars=list('qwertyupasdfghjkzxcvbnm23456789')): #string.ascii_lowercase + string.digits):
    return ''.join(random.choice(chars) for _ in range(size))


def gen_captcha(save_path):

        text = gen_string()
        image = generate_captcha(text)
        image = image.resize((135,50), Image.BILINEAR)
        filename = text + '_' + str(uuid.uuid4()) + ".png"
